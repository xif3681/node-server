...
// 删除任务
router.delete('/delTask', function (req, resp, next) {
    var id = req.query.id;
    var wherestr = { '_id': id };                 //条件
    Task.remove(wherestr, function (err, res) {
        if (err) {
            console.log("Error:" + err);
            resp.send({ mess: '删除失败！' });
        }
            ...
            for (let index = 0; index < SUCCESS.length; index++) {
                ...
                Task.update(wherestr, updatestr, function (err, res) {
                    if (err) {
                        ...
                        console.log('// 终端任务执行成功->服务器成功应答:');
                        let result1 = { 'server_time': UTC, 'task': result }
                        console.log('result1');
                        console.log(result1);
                        resp.send(result1);
